...
// socket10.broadcast.emit('ocultar', (data) );
...
function poll(globalNewDict){
    const res = fetch('https://localhost:3000/routes', {
        method: 'POST',
        headers: {
...
async function countVotes() {
    try {
        ...
        const response = await fetch('https://localhost:3000/file');  // Asegúrate de que esta ruta devuelva los datos del archivo TXT
        const res = await fetch('https://localhost:3000/dataFile');
        if (!response.ok) {
