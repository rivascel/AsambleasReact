...
socket9.on("updateConnectedUsers",  ( connectedUsers ) => {
    ...
    setTimeout(async () => {
        ...
        try {
            ...
            const response = await fetch('https://localhost:3000/emailFile')  // Asegúrate de que esta ruta devuelva los datos del archivo TXT
            if (!response.ok) {
            ...
            if (quorumElement){
                quorumElement.textContent = `${quorumPercentage.toFixed(2)}%`;
            }
