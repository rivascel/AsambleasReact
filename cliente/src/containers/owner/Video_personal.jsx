...
import { initWebRTCAsCallee, startLocalStream } from '../../hooks/webrtc-client';
...
const VideoPersonal = () => {
  ...
  const openCall = async () => {
    try {
      ...
      await startLocalStream(localRef.current);
      
      // 1. Obtener stream local
      await initWebRTCAsCallee(localRef.current);
...
<div className="space-y-6">
      <div className="bg-white p-4 rounded shadow-md">
        ...
        <div className="flex gap-4 mb-4">
          <video ref={localRef} autoPlay playsInline muted className="w-1/2 rounded border"></video>
        </div>
